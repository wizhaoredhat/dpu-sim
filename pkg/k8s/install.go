...
func (m *K8sMachineManager) GetLinuxDistro(machineIP string) (*linux.Distro, error) {
	return linux.Detect(m.sshClient, machineIP)
}
...
func (m *K8sMachineManager) installCRIO(machineIP, k8sVersion string) error {
	...
	sb := strings.Builder{}
	sb.WriteString("set -e\n")
	if m.linuxDistro.PackageManager == linux.DNF {
...
func (m *K8sMachineManager) SetupOVNBrEx(machineIP string, mgmtIP string, k8sIP string) error {
	...
	sb.WriteString("IF1_CONN=$(nmcli -g GENERAL.CONNECTION device show $IF1)\n")
	...
	sb.WriteString("nmcli conn delete $IF2\n")

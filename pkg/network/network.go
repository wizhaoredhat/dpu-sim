...
// Format: h<short-hash> where hash is from "hostName-dpuName"
func GenerateBridgeName(hostName, dpuName string) string {
	...
	// Take first 6 characters of hex hash for short identifier
	shortHash := fmt.Sprintf("%x", hash[:3])

	return fmt.Sprintf("h%s", shortHash)
}
...
func GetHostToDPUNetworkName(hostName, dpuName string) string {
	return fmt.Sprintf("%s-to-%s", hostName, dpuName)
}

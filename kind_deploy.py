...
class KindDeployer:
    ...
    def cleanup_cluster(self, cluster_name: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"  ✗ Failed to delete cluster: {result.stderr}")
                return False
    ...
    def cleanup_all(self) -> None:
        ...
        for cluster in clusters:
            ...
            self.cleanup_cluster(cluster_name)

        # Also cleanup kubeconfig files
        if self.kubeconfig_dir.exists():
            for f in self.kubeconfig_dir.glob('*.yaml'):
                print(f"  Removing kubeconfig: {f.name}")
                f.unlink()
